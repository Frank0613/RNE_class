...
def main():
    ...
    agent = PPO(
        policy_net, value_net, lr, max_grad_norm, clip_val, sample_n_epoch, sample_mb_size, mb_size, device=device
    )
    ...
    for it in range(start_it, n_iter):
        ...
        # Train
        pg_loss, v_loss = agent.train(mb_obs, mb_actions, mb_values, mb_advs, mb_returns, mb_old_a_logps)
